...
class DesignationList extends AbstractTable
{
    ...
    protected $headers = array(
             ...
             //'designationId' =>     array('tableAlias' => 'desgn',  'title' => 'Id' ) ,
             'designationName' =>        array( 'tableAlias' => 'desgn', 'title' => 'Designation Name' , 'filters' => 'text'),
             'status' =>     array( 'tableAlias' => 'desgn',  'title' => 'Status' ) ,
...
'<span class="userEditOptionTxt"><a href="/admin/user/designationedit/%s">Modify Designation</a></span>'.
 '<span class="userEditOptionTxt status"  id="%s"><a  href="javascript:void(0)" >Change Status</a></span>'.                  
'</span>'
           . '</span>' 
                         ,
            'vars' => array( 'designationId','designationId')
...
$this->getHeader('status')->getCell()->addDecorator('mapper', array(
    ...
    protected function initFilters($arrayData)
    {
         ...
         foreach($arrayData as $key => $row){    
            if ($value = $this->getParamAdapter()->getValueOfFilter('designationName')) {
                if(stripos($row['designationName'], $value) === false && !isset($keys[$key]) ){
                    $keys[] = $key;
                }
            }
       ...
       <script>
        ...
        $(".status").click(function (){
            ...
            if(yes)
            {
                ...
                var myData = {textData:getValue,type:"Designation"};
             $.ajax({
                type:"POST",
