...
class DocumentTypeList extends AbstractTable
{
    ...
    protected $headers = array(
      ...
      //       'docTypeId' =>     array('tableAlias' => 'doct',  'title' => 'Id' ) ,
             'docTypeName' =>        array( 'tableAlias' => 'doct', 'title' => 'Document Name' , 'filters' => 'text'),
             'status' =>     array( 'tableAlias' => 'doct',  'title' => 'Status' ) ,
...
'<span class="userEditOptionTxt"><a href="/admin/user/documentedit/%s">Modify Document</a></span>'.
 '<span class="userEditOptionTxt status"  id="%s"><a  href="javascript:void(0)" >Change Status</a></span>'.                  
'</span>'
           . '</span>' 
                         ,
            'vars' => array( 'docTypeId','docTypeId')
...
$this->getHeader('status')->getCell()->addDecorator('mapper', array(
    ...
    protected function initFilters($arrayData)
    {
         ...
         foreach($arrayData as $key => $row){    
            if ($value = $this->getParamAdapter()->getValueOfFilter('docTypeName')) {
                if(stripos($row['docTypeName'], $value) === false && !isset($keys[$key]) ){
                    $keys[] = $key;
                }
            }
       ...
       <script>
        $(".status").click(function (){
            ...
            if(yes)
            {
                ...
                var myData = {textData:getValue,type:"DocumentType"};
             $.ajax({
                type:"POST",
