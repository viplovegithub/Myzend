...
class AddressTypeList extends AbstractTable
{
    ...
    protected $headers = array(
           ...
           //  'bankId' =>     array('tableAlias' => 'b',  'title' => 'Id' ) ,
            'addrTypeName' =>        array( 'tableAlias' => 'addr', 'title' => 'Address Type' , 'filters' => 'text'),
            'status' =>     array( 'tableAlias' => 'addr',  'title' => 'Status' ) ,
...
'<span class="userEditOptionTxt"><a href="/admin/user/addresstypeedit/%s">Modify Address type</a></span>'.
 '<span class="userEditOptionTxt status"  id="%s"><a  href="javascript:void(0)" >Change Status</a></span>'.                  
'</span>'
           . '</span>' 
                         ,
            'vars' => array( 'addresstypeid','addresstypeid')
...
$this->getHeader('status')->getCell()->addDecorator('mapper', array(
    ...
    protected function initFilters($arrayData)
    {
         ...
         foreach($arrayData as $key => $row){    
            if ($value = $this->getParamAdapter()->getValueOfFilter('addrTypeName')) {
                if(stripos($row['addrTypeName'], $value) === false && !isset($keys[$key]) ){
                    $keys[] = $key;
                }
            }
       ...
       <script>
        $(".status").click(function (){
            ...
            if(yes)
            {
                ...
                var myData = {textData:getValue,type:"AddressType"};
             $.ajax({
                type:"POST",
