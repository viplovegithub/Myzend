...
class CompanyTypeList extends AbstractTable
{
   ...
   //Definition of headers
    protected $headers = array(
             ...
             //'usercatId' =>     array('tableAlias' => 'uc',  'title' => 'Id' ) ,
             'companyType' =>        array( 'tableAlias' => 'cc', 'title' => 'Company Type' , 'filters' => 'text'),
             'status' =>        array( 'tableAlias' => 'cc', 'title' => 'Status' , 'filters' => 'text'),
...
'<span class="userEditOptionTxt"><a href="/admin/user/companytypeedit/%s">Modify User Category</a></span>'.
'<span class="userEditOptionTxt status"  id="%s"><a  href="javascript:void(0)" >Change Status</a></span>'.               
'</span>'
                         ,
            'vars' => array( 'comptypeid','comptypeid')
    ...
    protected function initFilters($arrayData)
    {
        $keys = array();
         ...
         foreach($arrayData as $key => $row){    
            ...
            if ($value = $this->getParamAdapter()->getValueOfFilter('companyType')) {
                if(stripos($row['companyType'], $value) === false && !isset($keys[$key]) ){
                    $keys[] = $key;
                }
            }
      ...
      <script>
        $(".status").click(function (){
            ...
            if(yes)
            {
                ...
                var myData = {textData:getValue,type:"CompanyType"};
             $.ajax({
                type:"POST",
